...
export default {
  ...
  created() {
    ...
    this.$root.$on('openNewIMM', (id, name, num, imgSrc) => {
  ...
  mounted() {
    ...
    this.nimSdk = sdk.instance({
      ...
      // appId:"",
      userType: 'visitor',
      userTypeFlag: 'visitor',
      cilentToken: this.setToken(),
      platform: 'web',
      initSessionListGroupType: null,
      userId: this.$cookies.get('userId'),
      imUserId: this.$cookies.get('userId'),
      token: this.$cookies.get('userId'),
      sysCode: 'crisps-app',
  ...
  methods: {
    ...
    async createSession(id, name, num) {
      ...
      const imUserType = 'MERCHANT_B'
      // 1、根据规划师id创建对话
      // 2、根据创建的对话id查询 客户与该规划师之间的聊天记录
      await that.nimSdk.createSession(
        {
          imUserId: id,
          imUserType,
        },
    ...
    async getPlannerImUserId() {
      ...
      const headers = httpSign.handleSign({
        ...
        token: '', // 用户token
        sysCode: 'crisps-app', // P 必传, 平台sysCode
